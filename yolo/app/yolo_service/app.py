...
def get_best_structure_crop(image: np.ndarray, detections: List[Dict[str, Any]], base_filename: str) -> Optional[str]:
    """
    从检测结果中找到面积最大的目标并保存裁剪图
    """
    best_det = None
    max_area = 0

    for det in detections:
        # 筛选化合物类别（根据你的模型，这里假设 class_id 为 0）
        if det["class_id"] == 0:
            x1, y1, x2, y2 = det["bbox"]
            area = (x2 - x1) * (y2 - y1)
            
            # 策略：选择面积最大的（通常最能代表主体结构）
            if area > max_area:
                max_area = area
                best_det = det
    ...
    crop = image[y1:y2, x1:x2]
    crop_filename = f"best_crop_{base_filename}.jpg"
...
@app.post("/detect_best_crop", summary="上传图片并直接返回面积最大的化合物裁剪图")
async def detect_best_crop_image(file: UploadFile = File(...)):
    """
    上传图片，识别其中面积最大的化合物，裁剪后直接返回图片流
    """
    if not file.content_type.startswith("image/"):
        raise HTTPException(status_code=400, detail="文件必须是图片格式")
    ...
    try:
        # 1. 保存原始上传图以便推理
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S_%f")
        temp_path = os.path.join(UPLOAD_DIR, f"temp_{timestamp}.jpg")
        ...
        # 3. 筛选并裁剪最好的目标 (面积最大)
        best_crop_path = get_best_structure_crop(original_img, detections, timestamp)

        # 4. 如果没找到目标，返回 404
        if not best_crop_path or not os.path.exists(best_crop_path):
            raise HTTPException(status_code=404, detail="未在图片中检测到化合物结构")

        # 5. 直接返回图片文件流
        return FileResponse(best_crop_path, media_type="image/jpeg")
