...
"""
用途：
- 将 SMILES 批量/单条转换为 黑白 SVG/PNG
- 支持聚合物标记：{+n}
- 两种聚合物模式：
  A：--polymer_style bracket
     画布固定位置加括号（最稳通用）
  B：--polymer_style repeat_unit
     以 {+n} 的真实位置为锚点：
       1) 解析前把 {+n} 替换成 [*:999]（哑原子 + 映射号）
       2) 用锚点两侧邻接原子连线旋到水平（极稳）
       3) 括号包住重复单元：优先 repeat_smarts，否则 BFS 邻域
     且：不画括号中间“-”，括号尽量不压字（bbox 含 text + 留白）

重要修复：
- 彻底去掉 “小红框/红色小方块”：绘图前清掉所有 atom map number（包括 999）
  这些红框本质上是 RDKit 显示 atom map 的样式。

示例（批量）：
python smiles_img.py -i input.csv -o out_svg --format svg --polymer_style repeat_unit --robust --layout_tries 20 --repeat_radius 4
"""
...
def clean_smiles_text(s: str) -> str:
    """只保留 SMILES 主体；若含 CXSMILES 的 |...|，仅取 '|' 前面。"""
    if not s:
        return ""
    s = s.strip()
    if not s:
...
# 2) 聚合物锚点：{+n} -> [*:999]
# =========================
POLY_ANCHOR_MAP = 999  # 临时用来定位的原子映射号
...
def clear_all_atom_maps(mol: Chem.Mol):
    """清掉所有 atom map number（防止 RDKit 画出红框/红方块）。"""
    if mol is None:
        return
    for a in mol.GetAtoms():
        if a.GetAtomMapNum():
...
# 3) 清理会导致奇怪标记的属性
# =========================
def strip_atom_notes(mol: Chem.Mol) -> Chem.Mol:
    """清理别名/注释，避免 SVG 出现奇怪字符。"""
    if mol is None:
        return mol
    for a in mol.GetAtoms():
...
# =========================
# 9) BFS 邻域：repeat_smarts 缺失/失败时用
# =========================
...
# =========================
# 10) SVG：读取 viewBox / 插入 overlay / bbox（包含 text）
# =========================
...
def svg_bbox_for_atom_indices(svg: str, atom_indices: Set[int]) -> Optional[Tuple[float, float, float, float]]:
    """用 atom-<idx> 相关 path + text 一起求 bbox（包含字母位置，避免括号压字）。"""
...
def add_polymer_brackets_repeat_unit(svg: str, atom_set_for_unit: Set[int], label: str = "n") -> str:
    vb = get_viewbox(svg)
    if vb is None:
        return svg
    minx, miny, w, h = vb
...
def generate_image(raw_smiles: str,
    ...
    try:
        ...
        # ★关键修复：定位完锚点后，把所有 atom map 清掉（否则 RDKit 会画红框）
        clear_all_atom_maps(mol2d)

        # 动态调 bondLength/padding
        ...
        if fmt == "svg":
            svg = draw_svg(mol2d, width, height, metal_mode, bond_length, padding, anchor_idx=None)
            ...
            if is_poly and polymer_style == "bracket":
                ...
                # 用 repeat_smarts 定位重复单元（多匹配则选靠近锚点的）
                ...
                # repeat_smarts 失败：用锚点邻居 BFS 邻域近似
                if not unit_atoms:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="将 SMILES 转换为黑白 SVG/PNG（稳定版）。修复：清除 atom map 造成的小红框/红方块。"
    )
    ...
    parser.add_argument("--repeat_smarts", default="", help="B模式：重复单元 SMARTS（建议给能唯一定位的）。")
    parser.add_argument("--repeat_radius", type=int, default=4, help="repeat_smarts 失败时 BFS 邻域深度（默认 4）。")
